      ...
      <q-card class="form-card">
        ...
        <q-form @submit.prevent="guardarServicio">
          <q-card-section>
            ...
            <div class="row q-col-gutter-md">
              <div class="col-12 col-sm-6">
                <q-select v-model="formulario.marca" label="Marca *"
                  outlined :options="marcas"
                  :rules="[reglaRequerido]"
                  class="q-mb-md" />

                <q-input v-if="formulario.marca === 'Otra'"
            ...
            <q-input v-if="formulario.reparacion.includes('Otros')"
              v-model="formulario.reparacionOtro"
              label="Escribe el tipo de reparación *"
            ...
            <q-select v-model="formulario.estadoEquipo"
              ...
              :rules="[reglaEstadoEquipo]"
              class="q-mb-md">
              <template v-slot:hint v-if="!puedeEntregarEquipo">
                Debes pagar el valor total del servicio para poder marcarlo como Entregado.
              </template>
            </q-select>
...
const puedeEntregarEquipo = computed(() => {
  ...
  if (formulario.value.estadoPago === 'Pagado') {
    ...
    totalPagado = Number(formulario.value.valorAbono || 0)
  }
...
function opcionEstadoEquipoDeshabilitada(opcion) {
  return opcion === 'Entregado' && !puedeEntregarEquipo.value
}

function reglaEstadoEquipo(valor) {
  const requerido = reglaRequerido(valor)
  if (requerido !== true) return requerido
  if (valor === 'Entregado' && !puedeEntregarEquipo.value) {
    return 'No puedes entregar el equipo hasta que se pague el valor total del servicio'
  }
  return true
}
...
function guardarServicio() {
  ...
  // El equipo solo puede entregarse cuando el valor total del servicio está pagado.
  if (formulario.value.estadoEquipo === 'Entregado' && !puedeEntregarEquipo.value) {
    return
  }
  ...
  if (editando.value) {
    ...
    if (idx !== -1) {
      ...
      servicios.value[idx] = {
        ...formulario.value,
        marca: formulario.value.marca === 'Otra'
          ? formulario.value.marcaOtra.trim()
          : formulario.value.marca,
        reparacion: formulario.value.reparacion.map(tipo =>
          tipo === 'Otros' && formulario.value.reparacionOtro.trim()
            ? formulario.value.reparacionOtro.trim()
            : tipo
        ),
...
function editarServicioPorId(id) {
  ...
  const marcaEsPersonalizada = servicio.marca && !marcas.value.includes(servicio.marca)
  const reparacionPersonalizada = reparacionFormateada.find(
    tipo => !tiposReparacion.includes(tipo)
  )

  formulario.value = {
    ...crearFormulario(),
    ...servicio,
    marca: marcaEsPersonalizada ? 'Otra' : (servicio.marca || ''),
    marcaOtra: marcaEsPersonalizada ? servicio.marca : '',
    reparacion: reparacionPersonalizada
      ? [...reparacionFormateada.filter(tipo => tiposReparacion.includes(tipo)), 'Otros']
      : reparacionFormateada,
    reparacionOtro: reparacionPersonalizada || ''
  }
  editando.value = true
